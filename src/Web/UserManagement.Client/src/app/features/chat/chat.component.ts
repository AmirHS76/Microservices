    ...
    <section class="chat-shell">
      <aside class="people-panel">
        <div class="panel-header">
          <div>
            <span class="eyebrow"
              ><ng-icon name="lucideMessageCircle" size="14" /> Private
              messages</span
            >
            <h2>Chat</h2>
          </div>
          <span class="connection" [class.online]="realtime.connected()">
            <ng-icon
              [name]="realtime.connected() ? 'lucideWifi' : 'lucideWifiOff'"
              size="16"
            />
      ...
      <section class="thread-panel">
        @if (selectedUser(); as user) {
          ...
          <div class="empty-thread">
            ...
            <p>
              Choose any authenticated user from the list to start a private
              chat.
            </p>
          </div>
        }
...
export class ChatComponent implements OnDestroy {
  ...
  constructor() {
    this.title.setTitle("Chat - User Management");
    this.realtime.clearStale();
    this.loadInitial();
    ...
    this.api
      ...
      .subscribe((response) => {
        ...
        this.messages.update((messages) =>
          this.mergeMessages([...messages, ...older]),
        );
  ...
  onScroll(event: Event): void {
    ...
    if (
      element.scrollTop < this.SCROLL_BOTTOM_THRESHOLD &&
      this.hasMore() &&
  ...
  private upsertMessage(message: ChatMessage): void {
    ...
    const belongsToActive =
      ...
      ((message.senderId === this.currentUserId() &&
        message.recipientId === active.userId) ||
        (message.recipientId === this.currentUserId() &&
          message.senderId === active.userId));

    if (belongsToActive) {
      this.messages.update((messages) =>
        this.mergeMessages([...messages, message]),
      );
    }
  }
